...
export default async function CheckoutPage({ searchParams }: CheckoutPageProps) {
  ...
  const nextBillingDate = hasGracePeriod ? stoppedSub!.current_period_end : null;
  const isDiffUpgrade =
    hasGracePeriod &&
          ...
          <Card className="rounded-2xl border-muted shadow-sm">
            <CardContent className="p-6 flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex items-center justify-between text-sm">
                  <span className="text-muted-foreground font-medium">월 결제 금액</span>
                  <span className="font-bold text-foreground">₩{pricing.monthly.toLocaleString()}</span>
                </div>
                {isDiffUpgrade && (
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-muted-foreground font-medium">전환 방식</span>
                    <span className="font-bold text-foreground">고객센터 확인 필요</span>
                  </div>
                )}
                <div className="flex items-center justify-between text-sm">
              ...
              {hasGracePeriod ? (
                <div className="rounded-xl bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 p-4">
                  <p className="text-xs font-semibold text-blue-700 dark:text-blue-400 leading-relaxed">
                    ...
                    자동 결제가 시작됩니다. 지금은 카드만 등록하며 즉시 결제되지 않습니다.
                  </p>
                </div>
              ) : (
