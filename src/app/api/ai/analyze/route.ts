import { z } from "zod";
import { analyzeCompetition } from "@/services/ai-service";
...
export async function POST(request: Request) {
  ...
  const usage = await recordAndEnforce(user.userId, user.plan, "analyze", parsed.data.keyword);
  if (!usage.allowed) {
  ...
  try {
    const enrichment = await resolveEnrichment(user.userId, parsed.data.keyword, "analyze");
    const analysis = await analyzeCompetition(parsed.data.keyword, enrichment);
