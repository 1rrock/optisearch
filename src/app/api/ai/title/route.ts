...
export async function POST(request: Request) {
  ...
  try {
    const enrichment = await resolveEnrichment(user.userId, parsed.data.keyword, "title");
    const suggestions = await generateTitleSuggestions(parsed.data.keyword, parsed.data.context, enrichment);
