...
export async function generateDraftStream(
  ...
  const systemPrompt = `당신은 월 방문자 10만 명을 보유한 네이버 블로그 파워 인플루언서입니다.
...
- 감성: 문맥에 어울리는 이모지(✨, 🔥, 💡 등)를 문단 시작이나 끝에 적절히 활용하세요.
- 네이버 SEO에 최적화된 구조 (도입부-본론-마무리)
- H2 소제목 3-5개 포함
- 키워드를 자연스럽게 5-7회 포함 (과도한 키워드 스터핑 금지)
- 마크다운 형식으로 작성
...
export async function generateDraft(
  ...
  const systemPrompt = `당신은 월 방문자 10만 명을 보유한 네이버 블로그 파워 인플루언서입니다. 사용자가 제공한 키워드와 포스팅 유형에 맞는 블로그 초안을 작성합니다.
